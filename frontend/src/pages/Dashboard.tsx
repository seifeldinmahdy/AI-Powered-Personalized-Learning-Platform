...
import { Play, Clock, Award, TrendingUp, BookOpen, Target, Zap, Search } from 'lucide-react';
          ...
          <div className="p-8 max-w-7xl mx-auto">
            ...
            <div className="mb-8">
              <form onSubmit={handleSearch} className="relative">
                <div className="bg-gradient-to-r from-secondary/10 via-accent/10 to-primary/10 rounded-2xl p-1">
                  <div className="bg-card rounded-xl flex items-center">
                    <Search size={22} className="ml-5 text-muted-foreground flex-shrink-0" />
                    <input
                      type="text"
                    ...
                    <button
                      type="submit"
                      className="mr-2 px-6 py-2.5 bg-gradient-to-r from-secondary to-accent text-white rounded-xl text-sm font-semibold hover:shadow-lg transition-all"
                    >
                      Search
                    </button>
                  </div>
                ...
                <div className="flex items-center gap-2 mt-2.5 px-2">
                  ...
                  {['Python', 'JavaScript', 'Machine Learning'].map((tag) => (
                    <button
                      key={tag}
                      type="button"
                      onClick={() => { setSearchQuery(tag); navigate(`/courses?search=${encodeURIComponent(tag)}`); }}
                      className="text-xs px-2.5 py-1 bg-muted/50 hover:bg-muted rounded-lg text-muted-foreground hover:text-foreground transition-colors"
                    >
                      {tag}
                    </button>
                  ))}
                  <Link to="/courses" className="text-xs text-secondary hover:text-primary font-medium ml-auto transition-colors">
                    Browse all courses →
                  </Link>
                </div>
              </form>
